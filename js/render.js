...
function drawPlayerShip(c, x, y, angle, thrustMag, tick) {
  ...
  if (thrustMag > 0.05 && typeof NOZZLES !== "undefined" && typeof FLAME_CFG !== "undefined") {
    NOZZLES.forEach((nz, i) => {
      ...
      const len = thrustMag * FLAME_CFG.maxLen * flicker;
      const grad = c.createLinearGradient(nz.x, nz.y, nz.x, nz.y + len);
      grad.addColorStop(0, FLAME_CFG.colorCore);
      grad.addColorStop(1, FLAME_CFG.colorTip);
      c.beginPath();
      c.moveTo(nz.x - FLAME_CFG.width / 2, nz.y);
      c.lineTo(nz.x + FLAME_CFG.width / 2, nz.y);
  ...
  if (shipSprite.complete && shipSprite.naturalWidth) {
    ...
    c.drawImage(shipSprite, -SHIP_RENDER_W / 2, -SHIP_RENDER_H / 2, SHIP_RENDER_W, SHIP_RENDER_H);
